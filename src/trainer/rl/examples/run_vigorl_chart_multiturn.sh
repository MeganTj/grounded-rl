...
domain="chart" # web_grounding, vstar, spatial, web_action

# condition="vigorl_multiturn" # vanilla_thinking, vigorl, vigorl_multiturn
condition="vanilla_thinking" # vanilla_thinking, vigorl, vigorl_multiturn
condition=${1:-"vanilla_thinking"}
...
if [ "$domain" == "web_grounding" ]; then
    ...
    if [ "$condition" == "vanilla_thinking" ]; then
        ...
        MODEL_TAG=="multiturn_qwen2_5_vl_3b_osatlas"
        MULTITURN=true
...
elif [ "$domain" == "chart" ]; then

    if [ "$condition" == "vanilla_thinking" ]; then

        MODEL_PATH=Qwen/Qwen2.5-VL-3B-Instruct  # replace it with your local file path
        SYSTEM_PROMPT="./examples/format_prompt/chart_naive.jinja" # replace with the prompt for your dataset
        MODEL_TAG="vanilla_thinking_qwen2_5_vl_3b"
    ...
    elif [ "$condition" == "grounded_thinking" ]; then

        MODEL_PATH=Qwen/Qwen2.5-VL-3B-Instruct  # replace it with your local file path
        SYSTEM_PROMPT="./examples/format_prompt/chart_grounded_thinking.jinja" # replace with the prompt for your dataset
        MODEL_TAG="grounded_thinking_qwen2_5_vl_3b"
        REWARD_FUNCTION=./examples/reward_function/chart_grounded_thinking.py:compute_score_with_error
    elif [ "$condition" == "bbox" ]; then

        MODEL_PATH=Qwen/Qwen2.5-VL-3B-Instruct  # replace it with your local file path
        SYSTEM_PROMPT="./examples/format_prompt/chart_bbox.jinja" # replace with the prompt for your dataset
        MODEL_TAG="bbox_qwen2_5_vl_3b"
        REWARD_FUNCTION=./examples/reward_function/chart_bbox.py:compute_score_with_error
    elif [ "$condition" == "multiturn_bbox" ]; then

        SYSTEM_PROMPT="./examples/format_prompt/chart_multiturn_bbox.jinja" # replace with the prompt for your dataset
        # MODEL_TAG="vigorl_multiturn_qwen2_5_vl_3b_osatlas"
        MODEL_TAG=="multiturn_qwen2_5_vl_3b"
        MULTITURN=true
        MODEL_PATH=Qwen/Qwen2.5-VL-3B-Instruct
        # MODEL_PATH="gsarch/ViGoRL-Multiturn-MCTS-SFT-3b-Web-Grounding" # gsarch/ViGoRL-Multiturn-7b-Web-Grounding
        CROP_SIZE=384
    ...
    train_file="$DATA_ROOT/chart/verified_with_annot_default/questions_multistep_descriptive_train.jsonl"
    val_file="$DATA_ROOT/chart/verified_with_annot_default/questions_multistep_descriptive_test.jsonl"
